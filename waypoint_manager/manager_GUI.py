...
class Application(tk.Frame):
    def __init__(self, master):
        ...
        self.file_menu = tk.Menu(self.menu_bar, tearoff=tk.OFF)  # Create a menu to add to the menu bar
        ...
        self.file_menu.add_command(
            label="Save", command=self.menu_save, accelerator="Ctrl+S", state=tk.DISABLED  # Set callback function
        )
        self.file_menu.add_command(label="Save As", command=self.menu_saveas, accelerator="Ctrl+Shift+S", state=tk.DISABLED)
        ...
        self.fp_menu.add_checkbutton(
            label="Show footprint", command=self.menu_show_footprint, variable=self.show_fp, accelerator="Ctrl+F"
        )
        ...
        self.master.configure(menu=self.menu_bar)  # Set the menu bar created at the master
        ...
        self.mouse_position = tk.Label(self.status_bar, relief=tk.SUNKEN, text=" (x, y) = ", anchor=tk.W, font=("Consolas", 15))
        self.mouse_position.pack(side=tk.LEFT, padx=3)
        self.waypoint_num = tk.Label(
            self.status_bar, relief=tk.SUNKEN, text=" Waypoint No. -----", anchor=tk.W, font=("Consolas", 15)
        )
        ...
        self.editing_waypoint_id = None  # Object ID of the shape indicating the waypoint being edited
        self.moving_waypoint = False  # Whether you are in the process of moving waypoints
    ...
    def menu_open_map(self):
        map_path = tkinter.filedialog.askopenfilename(
            parent=self.master, title="Select map yaml file", initialdir=str(Path(".")), filetypes=[("YAML", ".yaml")]
        )
        if not map_path:
            return
    ...
    def menu_open_waypoints(self):
        ...
        filepath = tkinter.filedialog.askopenfilename(
            parent=self.master, title="Select waypoints yaml file", initialdir=str(Path(".")), filetypes=[("YAML", ".yaml")]
        )
        if not filepath:
            return
    ...
    def plot_waypoints(self, id=None):
        ...
        self.finish_pose.id = self.canvas.create_line(
            x0, y0, x1, y1, tags="finish_pose", width=10, arrow=tk.LAST, arrowshape=(12, 15, 9), fill="#AAF"
        )
        # trajectory
    ...
    def draw_trajectory(self):
        if self.canvas.find_withtag("trajectory"):
            ...
            self.canvas.create_line(list(itertools.chain.from_iterable(self.trajectory)), fill="#DF2", width=2, tags="trajectory")
    ...
    def disp_waypoint_info(self, id):
        ...
        apply_btn = tk.Button(canv, text="Apply", width=5, height=1, bg="#FDD", command=self.apply_btn_callback)
        ...
        remove_btn = tk.Button(canv, text="Remove", width=7, height=1, bg="#F00", command=self.remove_btn_callback)
    ...
    def remove_btn_callback(self):
        ...
        if yn == True:
            ...
            self.canvas.delete(self.editing_waypoint_id)  # Remove circle indicating waypoints
            self.close_wp_info()
    ...
    def mouse_move(self, event):
        ...
        if self.setting_finish_pose == 2:
            ...
            self.canvas.create_line(
                x0, y0, x1, y1, tags="set_finish_pose", width=10, arrow=tk.LAST, arrowshape=(12, 15, 9), fill="#F88"
            )
    ...
    def right_click(self, event):
        ...
        self.right_click_coord = [img_x, img_y]  # Stores the coordinates of the clicked source image in a variable
        return
    ...
    def draw_image(self):
        if not self.mymap:
            return
        if not self.canvas.find_withtag("map_image"):  # When drawing an image for the first time
            self.canvas.create_image(
                0, 0, anchor="nw", tags="map_image", image=self.mymap.get_draw_image((self.canv_w, self.canv_h))
            )
        else:
